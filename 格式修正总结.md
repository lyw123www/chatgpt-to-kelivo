...
## 重要修复：避免 blockquote 冲突

### 问题描述
Kelivo 的解析器使用 `line.startsWith('>')` 来识别角色标记。如果 Markdown 内容中包含引用块（blockquote），会导致消息被错误拆分。
...
这里的 `> **⚠️ 重要提醒：**` 会被误认为是新的角色标记，导致一条消息被拆分成多条。

### 解决方案
修改 `htmlToMarkdown` 函数中的 blockquote 处理：
...
4. **改进 Kelivo 解析器**
   - 更严格的角色标记识别（例如：只识别 `> 用户` 和 `> 助手`，忽略其他 `>` 开头的行）
   - 支持转义字符
